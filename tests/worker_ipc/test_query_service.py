...
from ppi.worker_ipc.query_service import (
    ALLOWED_QUERY_NAMES,
    QueryService,
...
def test_dict_normalization() -> None:
    result = normalize_query_result({"key": "value", "count": 42})
    assert list(result.columns) == ["key", "count"]
    assert result.row_count == 1
    assert result.truncated is False


def test_list_normalization() -> None:
    result = normalize_query_result([{"a": 1}, {"a": 2}])
    assert list(result.columns) == ["a"]
    assert result.row_count == 2
    assert result.truncated is False


def test_scalar_normalization() -> None:
...
def test_limit_truncation() -> None:
    result = normalize_query_result([{"a": i} for i in range(10)], limit=3)
