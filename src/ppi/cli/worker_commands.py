...
def cmd_start(workspace_id: str, analysis_dir, repo, profile, json_output: bool) -> None:
    ...
    if result.status == "healthy":
        ...
        if json_output:
            ...
            click.echo(f"Worker started: {result.worker_id or 'unknown'}")
    else:
        msg = f"Worker start failed: {result.message}"
        if json_output:
...
def cmd_status(repo, profile, analysis_dir, json_output: bool) -> None:
    ...
    if result.status == "healthy" and result.client is not None:
        asyncio.run(result.client.close())
    data = {
    ...
    if json_output:
        ...
        click.echo(f"Worker status: {result.status}")
