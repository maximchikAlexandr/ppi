...
6. **Given** the extension is installed and a workspace is open, **When** the analyst looks at the VS Code UI, **Then** they can discover PPI through a persistent visible entrypoint (Activity Bar view, Explorer context action, or status bar) without knowing command names in advance.
...
- **FR-025**: The extension MUST expose at least one persistent visible IDE entrypoint after installation (Activity Bar view, Explorer context action, editor title action, or status bar). Command Palette-only discoverability is not sufficient: the analyst MUST be able to discover PPI without knowing command names in advance.
- **FR-026**: The dashboard panel MUST retain its UI state (filters, selected commit, graph settings) when moved or docked between panel groups, either by persisting state through `vscode.getState()`/`setState()` in the frontend or by retaining the webview context when hidden.
- **FR-027**: The Webview message bridge MUST allowlist the `ppi rpc` methods the dashboard may invoke; any method not in the closed allowlist MUST be rejected so a compromised or stale webview cannot trigger arbitrary backend operations.
...
- To feed the panel without a server, the extension spawns one long-lived read-only CLI query process (`ppi rpc`) per open dashboard panel, which exposes the same read surface as `ppi serve` over a stdio JSON-RPC protocol; the extension bridges frontend reads to it via the message bridge. This is a read-only servant, NOT the Stage 7 worker: it performs no writes, owns no analysis, and its lifecycle is owned by the extension. Stage 5 uses this servant purely as an adapter from the existing CLI query surface to the Webview message bridge; it is not the future worker runtime and does not own analysis/storage. No HTTP server endpoint is introduced for the panel; only the CLI's existing read surface (shared with `ppi serve`) and the frontend's results-ingest adapter are used.
- The extension MUST declare the lowest VS Code engine version required by the actually used APIs (`engines.vscode`) and MUST be tested in the target Cursor version. The current APIs in use (commands, status bar, webview panel, output channel, workspace configuration, child process, tree view) require VS Code `^1.90.0` or later; raising the floor requires a concrete API need.
